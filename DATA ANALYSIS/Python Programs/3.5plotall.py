...
for file in files: # iterate through all the files
    ...
    for key, circle in location_dict.items():
        ...
        circle_patch = patches.Circle((x, y), scaled_radius, edgecolor='r', facecolor=facecolor, label=key)
    ...
    t = ('' +str(file))
    plt.text(75, 0, t, fontsize=14, style='oblique', ha='center',
            va='bottom', wrap=True)
    ax.imshow(img)
    # ax.legend(loc='upper right')  # Add legend for labels
    #plt.show()
    plt.savefig((file+'2.jpg'), dpi=300, quality=80, optimize=True, progressive=True)